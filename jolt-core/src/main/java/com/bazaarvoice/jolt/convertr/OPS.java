...
public enum OPS {
    ...
    public static OPS parse( String key ) {
        if ( key.contains( Convertr.WildCards.STAR ) ){

            if ( ! Convertr.WildCards.STAR.equals( key ) ) {
                throw new SpecException("Convertr key " + key + " is invalid.  * keys can only contain *, and no other characters." );
            }

            return STAR;
        }
        if ( key.contains( Convertr.WildCards.OR ) ) {
            return OR;
        }
        return LITERAL;
    }
    ...
    public static class OpsPrecedenceComparator implements Comparator<OPS> {
        /**
         * The order we want to apply Convertr logic is Literals, Or, and then Star.
